...
export const colors = {
  ...
  // ── Красный: только точка-индикатор перерасхода ──
  /**
   * oklch(0.55 0.16 25). Используется ТОЛЬКО как точка 6px рядом со строкой
   * категории. Ни фонов, ни текста сумм, ни полос прогресса этим цветом.
   */
  overspendDot: "#BD413F",
} as const;

/** Размер точки-индикатора перерасхода. */
